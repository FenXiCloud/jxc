...
public class CustomersCategoryController {
    ...
    public JsonResult delete(@PathVariable Integer customersCategoryId, @SaMerchantId Integer merchantId) {
        customersCategoryService.delete(customersCategoryId, merchantId);
        return JsonResult.successful();
    }

    @GetMapping("select")
    public JsonResult select(@SaMerchantId Integer merchantId) {
        return JsonResult.successful(customersCategoryService.select(merchantId));
    }
}
