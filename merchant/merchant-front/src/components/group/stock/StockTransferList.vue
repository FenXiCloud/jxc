  ...
  <div class="frame-page flex flex-column">
    ...
    <div class="flex1">
      <vxe-table row-id="id"
        ...
        <vxe-column title="调拨单号" field="code" width="200"/>
        <vxe-column title="调入仓库" field="inWarehouseName" min-width="120"/>
        <vxe-column title="调出仓库" field="outWarehouseName" min-width="120"/>
        <vxe-column title="创建时间" field="createDate" align="center" width="200" sortable/>
        <vxe-column title="调拨日期" field="billDate" align="center" width="130" sortable/>
...
import {h} from "vue";
import StockTransferForm from "@components/group/stock/StockTransferForm.vue";
import StockTransferView from "@components/group/stock/StockTransferView.vue";
...
export default {
  ...
  methods: {
    ...
    loadList(type = true) {
      this.loading = true;
      // this.loadTotal(type)
      StockTransfer.list(this.queryParams).then(({data: {results, total}}) => {
      ...
      if (records.length == 0) {
        ...
        if (params.length) {
          confirm({
            ...
            onConfirm: () => {
              StockTransfer.batchState(params).then(() => {
                message("操作成功~");
      ...
      if (records.length == 0) {
        ...
        if (params.length) {
          confirm({
            ...
            onConfirm: () => {
              StockTransfer.batchPay(params).then(() => {
                message("操作成功~");
                $table.clearCheckboxRow()
                this.loadList();
              })
    ...
    updatePayState(row) {
      confirm({
        ...
        onConfirm: () => {
          StockTransfer.updatePayState(row.id).then(() => {
            message("操作成功~");
            this.loadList();
          })
        }
    ...
    updateState(row, stateName, state) {
      confirm({
        title: "系统提示",
        content: `${stateName}：-调拨单号：${row.code}?`,
        onConfirm: () => {
          StockTransfer.updateState({id: row.id, purchaserOrderState: state}).then(() => {
            message("操作成功~");
            this.loadList();
          })
        }
    ...
    doRemove(row) {
      confirm({
        ...
        onConfirm: () => {
          StockTransfer.remove(row.id).then(() => {
            message("删除成功~");
            this.loadList();
          })
        }
