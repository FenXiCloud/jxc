  ...
  <div class="frame-page flex flex-column">
    ...
    <div class="flex1">
      <vxe-table row-id="id"
        ...
        <vxe-column title="其他入库单号" field="code" width="200"/>
...
export default {
  ...
  methods: {
    ...
    loadList(type = true) {
      this.loading = true;
      // this.loadTotal(type)
      StockInbound.list(this.queryParams).then(({data: {results, total}}) => {
      ...
      if (records.length == 0) {
        ...
        if (params.length) {
          confirm({
            ...
            onConfirm: () => {
              StockInbound.batchState(params).then(() => {
                message("操作成功~");
      ...
      if (records.length == 0) {
        ...
        if (params.length) {
          confirm({
            ...
            onConfirm: () => {
              StockInbound.batchPay(params).then(() => {
                message("操作成功~");
                $table.clearCheckboxRow()
                this.loadList();
              })
    ...
    updatePayState(row) {
      confirm({
        ...
        onConfirm: () => {
          StockInbound.updatePayState(row.id).then(() => {
            message("操作成功~");
            this.loadList();
          })
        }
    ...
    updateState(row, stateName, state) {
      confirm({
        ...
        onConfirm: () => {
          StockInbound.updateState({id: row.id, purchaserOrderState: state}).then(() => {
            message("操作成功~");
            this.loadList();
          })
        }
    ...
    doRemove(row) {
      confirm({
        ...
        onConfirm: () => {
          StockInbound.remove(row.id).then(() => {
            message("删除成功~");
            this.loadList();
          })
        }
