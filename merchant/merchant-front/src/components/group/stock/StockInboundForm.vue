    ...
    <div class="modal-column-full-body">
      <vxe-toolbar class-name="!size--mini">
        <template #buttons> <label class="mr-20px ml-16px" style="font-size: 16px !important;">单据日期：</label>
          ...
          <Select class="w-160px" filterable required :deletable="false" v-model="form.businessType"  :datas="{其他入库:'其他入库',盘盈:'盘盈'}" placeholder="请选择业务类型"/>
        </template>
      </vxe-toolbar>
      <vxe-table
          size="mini"
        ...
        <vxe-column title="入库单位" field="orderUnitName" align="center" width="80">
          <template #default="{row,rowIndex}">
        ...
        <vxe-column title="入库单价" field="orderPrice" width="100">
          <template #default="{row,rowIndex}">
        ...
        <vxe-column title="入库金额" field="discountedAmount" width="100">
          <template #default="{row,rowIndex}">
...
export default {
  ...
  methods: {
    ...
    confirm() {
      ...
      StockInbound.save({order: Object.assign(this.form, {discountedAmount: this.discountedAmount}), type: this.type, detailList: productsData}).then(() => {
        message("保存成功~");
      }).finally(() =>
              this.clear(),
          loading.close());
