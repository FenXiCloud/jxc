...
export default {
  ...
  methods: {
    ...
    loadList(type = true) {
      this.loading = true;
      this.loadTotal(type)
      PurchaseRtOrder.list(this.queryParams).then(({data: {results, total}}) => {
      ...
      if (records.length == 0) {
        ...
        if (params.length) {
          confirm({
            ...
            onConfirm: () => {
              PurchaseRtOrder.batchState(params).then(() => {
                message("操作成功~");
      ...
      if (records.length == 0) {
        ...
        if (params.length) {
          confirm({
            ...
            onConfirm: () => {
              PurchaseRtOrder.batchPay(params).then(() => {
                message("操作成功~");
                $table.clearCheckboxRow()
                this.loadList();
              })
    ...
    updatePayState(row) {
      confirm({
        ...
        onConfirm: () => {
          PurchaseRtOrder.updatePayState(row.id).then(() => {
            message("操作成功~");
            this.loadList();
          })
        }
    ...
    updateState(row, stateName, state) {
      confirm({
        ...
        onConfirm: () => {
          PurchaseRtOrder.updateState({id: row.id, purchaserOrderState: state}).then(() => {
            message("操作成功~");
            this.loadList();
          })
        }
