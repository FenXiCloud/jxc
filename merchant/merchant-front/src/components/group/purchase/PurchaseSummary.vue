  ...
  <div class="frame-page flex flex-column">
    <vxe-toolbar>
      <template #buttons>
        ...
        <Select class="w-200px ml-2px" :datas="warehouses" keyName="id" filterable titleName="name" v-model="warehousesId" placeholder="仓库" :multiple="true"/>
    ...
    <div class="flex1">
      <vxe-table row-id="id"
        ...
        <vxe-column title="仓库" field="warehouseName" width="160"/>
        ...
        <vxe-column title="退货总金额" field="returnAmount" width="160"/>
      </vxe-table>
    </div>
...
export default {
  ...
  computed: {
    ...
    downParams() {
      return Object.assign(this.params, {
        customersIds: this.warehousesId.map(item => item).toString(),
        productsIds: this.productsIds.map(item => item).toString(),
        start: manba(this.dateRange.start).format("yyyy-MM-dd"),
        end: manba(this.dateRange.end).format("yyyy-MM-dd"),
      })
  ...
  methods: {
    footerMethod({columns, data}) {
      ...
      columns.forEach((column) => {
        if (column.property && ['discountedAmount','returnAmount'].includes(column.property)) {
          let total = 0;
          data.forEach((row) => {
    ...
    loadList(type = true) {
      if (this.dateRange.start && this.dateRange.end) {
        this.loading = true;
        OrderReport.purchaseByProducts(this.queryParams).then(({data: {results, total}}) => {
  ...
  created() {
    Promise.all([
      ...
      this.warehouses = results[0].data;
      this.productsList = results[1].data;
    });
    this.loadList()
  },
}
